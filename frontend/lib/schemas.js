...
// JSON output from Claude via tool-use.
//
// ESM mirror of backend/lib/schemas.js, kept inside `frontend/` because the
// Vercel project root is `frontend/` and serverless functions there cannot
// import code from outside that root. The local Express server uses the CJS
// copy under backend/lib/.

export const DOC_TYPES = ['invoice', 'contract', 'email', 'generic'];

export const SCHEMAS = {
  invoice: {
    toolName: 'extract_invoice',
    description: 'Extract structured data from an invoice or receipt document.',
    input_schema: {
      type: 'object',
      properties: {
