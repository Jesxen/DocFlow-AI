// extractor.js
// Core extraction logic shared between the local Express server and the Vercel
// serverless function. Calls the Anthropic Messages API with tool-use to force
// strict structured JSON output (no markdown fences). Parses defensively.
...
// normalize, and enforce a hard length cap.
function sanitizeText(raw) {
  if (typeof raw !== 'string') return '';
  // Remove null bytes and most C0 control chars but keep newlines/tabs.
  // (Keeps \t \n \r; strips \x00-\x08, \x0B, \x0C, \x0E-\x1F.)
...
async function extract({ text, docType, apiKey }) {
  ...
  try {
    ...
    // Map common Anthropic SDK errors to user-friendly messages.
  ...
  try {
    ...
    if (!toolBlock || !toolBlock.input) {
      return {
        ok: false,
        status: 502,
        error: 'The service returned an unexpected response. Please try again.',
      };
    }
    ...
    return {
      ok: false,
      status: 502,
      error: 'Failed to parse the extraction result. Please try again.',
    };
  }
}

module.exports = { extract, sanitizeText, MODEL_ID, MAX_INPUT_CHARS };
